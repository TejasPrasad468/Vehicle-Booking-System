...
const AdminTaskCard = ({ task, variant }: AdminTaskCardProps) => {
  return (
        <div
            className={`relative overflow-hidden rounded-2xl shadow-xl p-7 text-white transform transition duration-300 hover:scale-105 hover:shadow-2xl ${variant} animate-fade-in-up`}
        >
            <div className="absolute right-4 top-4 opacity-20 text-6xl pointer-events-none select-none">👑</div>
            <h2 className="text-2xl font-bold mb-2 drop-shadow">{task.title}</h2>
            ...
            <div className="mb-5 flex items-center gap-2">
                ...
                <span className="bg-white/20 px-2 py-1 rounded text-xs">{task.assignedBy}</span>
            </div>
